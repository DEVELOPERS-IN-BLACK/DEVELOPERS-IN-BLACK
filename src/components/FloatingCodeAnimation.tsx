...
interface FloatingCodeAnimationProps {
  snippetCount?: number; // Số lượng snippet hiển thị, mặc định là 30
}

const FloatingCodeAnimation: React.FC<FloatingCodeAnimationProps> = ({ snippetCount = 30 }) => {
  const codeSnippets = [
    'const dev = "gentleman"',
    'console.log("DIB");',
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden">
      {codeSnippets.slice(0, Math.min(snippetCount, codeSnippets.length)).map((snippet, i) => {
        const { startX, startY } = generatePosition();
        return (
          <div
            key={`floating-${i}`}
